...
namespace util
{
        ...
        D3D12_DRED_DEVICE_STATE const&             deviceState)
    {
        ...
        if (pageFaults.pHeadExistingAllocationNode == nullptr) message << L"\t| No existing allocation node" << std::endl;

        if (pageFaults.pHeadRecentFreedAllocationNode == nullptr) message << L"\t| No recent freed allocation node" << std::endl;
