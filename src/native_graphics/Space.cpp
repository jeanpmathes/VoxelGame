...
void Space::CreateGlobalConstBuffer()
{
    ...
    m_globalConstantBufferMapping.Write(
        {.time = 0.0f, .textureSize = DirectX::XMUINT3{1, 1, 1}, .lightDirection = DirectX::XMFLOAT3{0.0f, -1.0f, 0.0f}, .minLight = 0.4f, .minShadow = 0.2f});
}
...

    {
        ...
        auto fillSlots = [&](ShaderResources::Table::Entry const entry, UINT const base, std::optional<UINT> const count)
        {
            if (count.has_value())
            {
                ...
                m_globalShaderResources->CreateShaderResourceView(entry, 0, {m_sentinelTexture->GetResource(), &m_sentinelTextureSRV});
        };
    ...
    nv_helpers_dx12::RayTracingPipelineGenerator& pipeline)
{
    ...
    auto compileShaderLibrary = [&](UINT const shader)
    {
        ...
        for (UINT symbolOffset = 0; symbolOffset < currentSymbolCount; symbolOffset++)
            symbols.emplace_back(pipelineDescription.symbols[currentSymbolIndex++]);
...
void Space::InitializeAnimations()
{
    for (auto& animation : m_animations)
        animation.Initialize(*m_client, m_globalShaderResources->GetComputeRootSignature());
}
...
void Space::CopyOutputToBuffers(Allocation<ID3D12Resource> const& color, Allocation<ID3D12Resource> const& depth) const
{
    ...
    GetCommandList()->ResourceBarrier(entry.size(), entry.data());
    ...
    GetCommandList()->ResourceBarrier(exit.size(), exit.data());
}
