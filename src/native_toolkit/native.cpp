...
NATIVE HRESULT NativeDeallocate(Allocator const* allocator, std::byte* const pointer) { return allocator->Deallocate(pointer); }
...
NATIVE FLOAT NativeGetNoise3D(Noise const* noise, FLOAT const x, FLOAT const y, FLOAT const z) { return noise->GetNoise(x, y, z); }
