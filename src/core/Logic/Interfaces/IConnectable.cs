...
namespace VoxelGame.Core.Logic.Interfaces
{
    public interface IConnectable : IBlockBase
    {
        ...
        /// <returns></returns>
        public static uint GetConnectionData<TConnectable>(World world, int x, int y, int z)
            where TConnectable : IConnectable
        {
            uint data = 0;

            if (world.GetBlock(x, y, z - 1, out _) is TConnectable north &&
                north.IsConnectable(world, BlockSide.Front, x, y, z - 1))
                data |= 0b00_1000;

            if (world.GetBlock(x + 1, y, z, out _) is TConnectable east &&
                east.IsConnectable(world, BlockSide.Left, x + 1, y, z))
                data |= 0b00_0100;

            if (world.GetBlock(x, y, z + 1, out _) is TConnectable south &&
                south.IsConnectable(world, BlockSide.Back, x, y, z + 1))
                data |= 0b00_0010;

            if (world.GetBlock(x - 1, y, z, out _) is TConnectable west &&
