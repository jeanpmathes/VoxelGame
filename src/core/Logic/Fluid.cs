...
public abstract partial class Fluid : IIdentifiable<uint>, IIdentifiable<string>
{
    ...
    {
        if (RenderType != RenderType.NotRendered &&
            (info.Block is IFillable { RenderFluid: true } ||
             info.Block is not IFillable && !info.Block.IsSolidAndFull))
        {
            ...
            void MeshFluidSide(BlockSide side)
            {
                ...
                bool meshAtNormal = (int) info.Level > sideHeight && blockToCheck?.IsOpaque != true;
