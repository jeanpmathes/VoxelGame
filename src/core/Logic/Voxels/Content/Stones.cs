...
public class Stones(BlockBuilder builder) : Category(builder)
{
    ...
    /// </summary>
    public Stone Granite { get; } = builder.BuildStone(nameof(Granite), Language.Granite);

    /// <summary>
    ///     Sandstone is found all over the world and especially in the desert.
    ///     When carved, the patterns depict the desert sun.
    /// </summary>
    public Stone Sandstone { get; } = builder.BuildStone(nameof(Sandstone), Language.Sandstone);
    ...
    /// </summary>
    public Stone Limestone { get; } = builder.BuildStone(nameof(Limestone), Language.Limestone);

    /// <summary>
    ///     Marble is a rarer stone type.
    ///     When carved, the patterns depict an ancient temple.
    /// </summary>
    public Stone Marble { get; } = builder.BuildStone(nameof(Marble), Language.Marble);
    ...
    /// </summary>
    public Stone Pumice { get; } = builder.BuildStone(nameof(Pumice), Language.Pumice);

    /// <summary>
    ///     Obsidian is a dark type of stone, that forms from lava.
    ///     When carved, the patterns depict an ancient artifact.
    /// </summary>
    public Stone Obsidian { get; } = builder.BuildStone(nameof(Obsidian), Language.Obsidian);
}
