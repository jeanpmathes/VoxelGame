...
///     In contrast to a <see cref="Visuals.Models.Model" />, the mesh is stored in a format ready to be uploaded to the GPU.
/// </summary>
public class Mesh
{
    ...
    private static void DivideAlongU(Int32 target, Mesh mesh, Quad original)
    {
        ...
        (Vector2 a, Vector2 b, Vector2 c, Vector2 d) uv = MeshData.GetUVs(ref original.data);
        Vector2 midLeftUV = (uv.a + uv.b) / 2;
        Vector2 midRightUV = (uv.d + uv.c) / 2;

        MeshData.SetUVs(ref mesh.quads[first].data, uv.a, midLeftUV, midRightUV, uv.d);
        MeshData.SetUVs(ref mesh.quads[second].data, midLeftUV, uv.b, uv.c, midRightUV);
    }
    ...
    private static void DivideAlongV(Int32 target, Mesh mesh, Quad original)
    {
        ...
        MeshData.SetUVs(ref mesh.quads[first].data, uv.a, uv.b, midTopUV, midBottomUV);
        MeshData.SetUVs(ref mesh.quads[second].data, midBottomUV, midTopUV, uv.c, uv.d);
    }
