...
namespace VoxelGame.Core.Visuals.Models;
...
public sealed partial class Model : IResource, ILocated
{
    ...
    private void ToData(out Mesh.Quad[] quads, ITextureIndexProvider textureIndexProvider, IReadOnlyDictionary<Int32, TID>? textureOverrides = null)
    {
        ...
        for (Int32 index = 0; index < Quads.Length; index++)
        {
            ...
            MeshData.SetTextureIndex(ref quads[index].data, textureIndexLookup[quad.TextureId]);

            MeshData.SetUVs(ref quads[index].data,
