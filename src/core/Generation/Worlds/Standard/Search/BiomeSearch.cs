...
public class BiomeSearch(Dictionary<String, Biome> biomes, Searcher searcher) : SearchCategory<Biome>(biomes, [InnerModifier, BorderModifier], searcher)
{
    ...
    {
        Int32 maxCellDistance = (Int32) Math.Clamp(maxBlockDistance / Map.CellSize + 1, min: 0, World.BlockLimit * 2 / Map.CellSize);

        for (Int32 distance = 0; distance < maxCellDistance; distance++)
