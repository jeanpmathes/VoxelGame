...
namespace VoxelGame.Core.Physics
{
    public static class Raycast
    {
        ...
        public static bool CastLiquid(Ray ray, out int hitX, out int hitY, out int hitZ, out BlockSide side)
        {
            ...
            // Check the current block.
            Liquid? currentLiquid = Game.World.GetLiquid(x, y, z, out LiquidLevel level, out _);

            // Check if the ray intersects the bounding box of the block.
            if (currentLiquid != null && currentLiquid != Liquid.None && Liquid.GetBoundingBox(x, y, z, level).Intersects(ray))
            {
                hitX = x;
                hitY = y;
                hitZ = z;
            ...
            while (!(x == endX && y == endY && z == endZ))
            {
                ...
                //Check the current block
                currentLiquid = Game.World.GetLiquid(x, y, z, out level, out _);
