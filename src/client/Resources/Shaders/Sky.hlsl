// <copyright file="Sky.hlsl" company="VoxelGame">
...
#ifndef VG_SHADER_SKY_HLSL
#define VG_SHADER_SKY_HLSL 
...
namespace vg
{
    float GetCelestialBodyIntensity(float3 rayDirection, float3 bodyDirection, float factor)
    {
        return min(pow(max(dot(rayDirection, bodyDirection), 0.0f), factor * 1000.0f), 1.0f);
    }
    ...
    /**
     * Get the color of the sky for the given ray direction.
     * @return 
     */
    float3 GetSkyColor(float3 rayDirection)
    {
        ...
        float const timeOfDay      = custom.timeOfDay;
