...
AddressableBuffer InBufferAllocator::Allocate(const UINT64 size)
{
...
{
    const size_t uavCount = resources.size() + m_blocks.size();
    ...
    const UINT barrierCount = static_cast<UINT>(uavCount);
    if (barrierCount == 0) return;

    commandList->ResourceBarrier(barrierCount, barriers.data());
}

AddressableBuffer InBufferAllocator::AllocateInternal(const UINT64 size)
{
...
std::unique_ptr<InBufferAllocator::Block> InBufferAllocator::Block::Create(
    InBufferAllocator& allocator, const size_t index)
{
...
std::optional<AddressableBuffer> InBufferAllocator::Block::Allocate(const D3D12MA::VIRTUAL_ALLOCATION_DESC* description)
{
...
void InBufferAllocator::Block::FreeAllocation(const D3D12MA::VirtualAllocation allocation)
{
...
InBufferAllocator::Block::Block(
    ...
    const size_t index)
    : block(block), memory(std::move(memory)), m_allocator(allocator), m_index(index)
{
}
...
AddressableBuffer::AddressableBuffer(
    const D3D12_GPU_VIRTUAL_ADDRESS address, const D3D12MA::VirtualAllocation allocation,
    InBufferAllocator::Block* block)
    : m_address(address), m_allocation(allocation), m_block(block)
{
}
...
D3D12_GPU_VIRTUAL_ADDRESS AddressableBuffer::GetAddress() const
{
    return m_address;
}
