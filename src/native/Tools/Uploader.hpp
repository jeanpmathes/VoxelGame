...
public:
     ...
     */
    Uploader(NativeClient& client, const ComPtr<ID3D12GraphicsCommandList>& optionalCommandList);
     ...
     */
    void UploadBuffer(const std::byte* data, UINT size, const Allocation<ID3D12Resource>& destination);

    /**
     * Execute the uploads.
     */
    void ExecuteUploads(const ComPtr<ID3D12CommandQueue>& commandQueue) const;
